...
```{r set-seed}
set.seed(101)                 # make sure to change the seed
```
...
The distribution is rightly skewed， with a mean of 1498.
...
ci99_data <- data.frame(ci99_id = c(1:50, 1:50),
                      ci99_bounds = c(ci99$lower, ci99$upper),
                      capture_mu = c(ci99$capture_mu, ci99$capture_mu))

ggplot(data = ci99_data, aes(x=ci99_bounds, y = ci99_id, group = ci99_id, color = capture_mu)) + 
  geom_point(size = 2) + 
  geom_line() +
