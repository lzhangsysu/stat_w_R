...
samp1 %>%
  summarise(s1mu = mean(area), s1pop_med = median(area), 
            s1sigma = sd(area), s1pop_iqr = IQR(area),
            s1pop_min = min(area), s1pop_max = max(area),
            s1pop_q1 = quantile(area, 0.25),  # first quartile, 25th percentile
            s1pop_q3 = quantile(area, 0.75))  # third quartile, 75th percentile
```
...
sample_means150 %>%
  summarise(mprice150 = mean(x_bar), 
            sdprice150 = sd(x_bar))  
```
...
sample_means50 %>%
  summarise(ma50 = mean(x_bar), 
            sda50 = sd(x_bar))  

sample_means150 %>%
  summarise(ma150 = mean(x_bar), 
            sda150 = sd(x_bar))
```
