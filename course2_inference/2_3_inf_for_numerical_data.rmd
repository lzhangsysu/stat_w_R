...
ggplot(data = subset(nc, !is.na(habit)), aes(x = habit, y = weight)) + 
  geom_boxplot() +
  xlab("Mother's Smoking Habbit") + ylab("Bady Weight at Birth (lbs)")
...
```
99% CI: (38.0952 , 38.5742); 90% CI: (38.1819 , 38.4874). 90%CI is narrower
...
QUESTION: Whether the baby's weight at birth for white vs non-white moms are different?

```{r pick-your-own}
# type your code for the Exercise here, and Knit
ggplot(data = subset(nc, !is.na(nc$whitemom)), aes(x = whitemom, y = weight)) + 
  geom_boxplot() +
  xlab("Mother's Race") + ylab("Bady Weight at Birth (lbs)")

inference(y = weight, x = whitemom, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
```

As the p-value is extremely small, we reject the null hypothesis that the weight of baby at birth for white mom and non-white mom are the same.
