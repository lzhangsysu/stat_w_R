...
```{r}
ggplot(nc_fullterm, aes(x = weight)) +
  geom_histogram(binwidth = 0.3)
```
...
bayes_inference(y = weight, data = nc_fullterm, 
                ...
                prior_family = "JZS", null = 7.7, rscale = 1,
                alternative = 'twosided', method = "theoretical", 
                cred_level = 0.95, show_plot = FALSE)
```
...
of the variable `weight` using the output of `bayes_inference` which we will save to the object `weight_post`:
...
We'll first convert our `samples` into a dataframe and then use `mutate` to create draws from the predictive distribution using `rnorm`:
...
In this case, since the posterior distribution of $\mu$ and $Y_{new}$ are both symmetric, we can set L to be the 0.025 quantile and U to be the 0.975 quantile. 
Using the `quantile` function `R` we can find the 0.025 and 0.975, as well as median (0.50) quantiles of the predictive distribution:
...
The distribution seems somewhat left-skewed.
...
BF[H2:H1] indicates evidence against H1 is very strong.

### Prediction using MCMC

use the output of `bayes_inference` and save to the object `weight_premie_post`:

```{r}
weight_premie_post = bayes_inference(y = weight, data = nc_premie, 
                statistic = "mean", type = "ci",  
                prior_family = "JZS", mu_0 = 7.7, rscale = 1,
                method = "simulation",
                cred_level = 0.95)
...
list the output or objects that are stored in the object created by `bayes_inference`:

```{r}
names(weight_premie_post)
```

Convert `samples` into a dataframe and then use `mutate` to create draws from the predictive distribution using `rnorm`:

```{r}
samples_pre = as.data.frame(weight_premie_post$samples)
nsim_pre = nrow(samples_pre)
samples_pre = mutate(samples_pre, y_pred_pre = rnorm(nsim_pre, mu, sqrt(sig2)))
```

View an estimate of the predictive distribution:

```{r}
ggplot(data = samples_pre, aes(x = y_pred_pre)) + 
  geom_histogram(aes(y = ..density..), bins = 100) +
  geom_density() + 
  xlab(expression(y[new]))
```

Since the posterior distribution of $\mu$ and $Y_{new}$ are both symmetric, set L to be the 0.025 quantile and U to be the 0.975 quantile. 
Using the `quantile` function `R` to find the 0.025 and 0.975, as well as median (0.50) quantiles of the predictive distribution:

```{r}
dplyr::select(samples_pre, mu, y_pred_pre) %>%
  map(quantile, probs=c(0.025, 0.50, 0.975))
```
...
tapply(nc_fullterm$weight, nc_fullterm$habit, summary)
```
...
bayes_inference(y = weight, x = habit, data = nc_fullterm, 
                ...
                type = "ci", prior = "JZS", 
                mu_0 = 0, rscale = 1, 
                method = "simulation", cred_level = 0.95)
```
...
tapply(pss_all$diff, pss_all$Group, summary)

bayes_inference(y = diff, x = Group, data = pss_all, statistic = 'mean',
