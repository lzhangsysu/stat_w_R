...
ggplot(data = wage, aes(x = educ, y = wage)) +
  geom_point(alpha = 0.4) +
  stat_smooth(method = "lm", se = FALSE) +
  ggtitle("wage vs education") + xlab("years of education") + ylab("weekly earnings")
...
ggplot(data = wage, aes(x = iq, y = lwage)) +
  geom_point(alpha = 0.4) +
  stat_smooth(method = "lm", se = FALSE) +
  ggtitle("wage vs iq") + xlab("iq") + ylab("log(weekly earnings)")
...
m_lwage_tenure = lm(lwage ~ tenure, data = wage)

tidy(m_lwage_tenure)

ggplot(data = wage, aes(x = tenure, y = lwage)) +
  geom_point(alpha = 0.4) +
  stat_smooth(method = "lm", se = FALSE) +
  ggtitle("wage vs tenure") + xlab("years with current employer") + ylab("log(weekly earnings)")

confint(m_lwage_tenure)

m_lwage_tenure_aug <- augment(m_lwage_tenure)

ggplot(data = m_lwage_tenure_aug, aes(x = .fitted, y = .resid)) +
  geom_point(alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Fitted values", y = "Residuals")

ggplot(data = m_lwage_tenure_aug, aes(x = .resid)) +
  geom_histogram(binwidth = 0.1) +
  xlab("Residuals")

ggplot(m_lwage_tenure_aug) +
  geom_qq(aes(sample = .std.resid)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(x = "Theoretical quantiles", y = "Standardized residuals")
...
m_lwage_kww = lm(lwage ~ kww, data = wage)

tidy(m_lwage_kww)

ggplot(data = wage, aes(x = kww, y = lwage)) +
  geom_point(alpha = 0.4) +
  stat_smooth(method = "lm", se = FALSE) +
  ggtitle("wage vs kww") + xlab("kww score") + ylab("log(weekly earnings)")

confint(m_lwage_kww)

m_lwage_kww_aug <- augment(m_lwage_kww)

ggplot(data = m_lwage_kww_aug, aes(x = .fitted, y = .resid)) +
  geom_point(alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Fitted values", y = "Residuals")

ggplot(data = m_lwage_kww_aug, aes(x = .resid)) +
  geom_histogram(binwidth = 0.1) +
  xlab("Residuals")

ggplot(m_lwage_kww_aug) +
  geom_qq(aes(sample = .std.resid)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(x = "Theoretical quantiles", y = "Standardized residuals")
```
...
outliers2 <- Bayes.outlier(m_lwage_iq, prior.prob=0.95)
```
...
outliers_df2 <- data.frame(probability = outliers2$prob.outlier,
                          case = 1:length(outliers$prob.outlier))
ggplot(outliers_df, aes(ymax = probability, x = case)) +
  geom_linerange(ymin = 0) +
  labs(y = "Probability")
...
```{r Q10}
# Type your code for Question 10 here. #using ZS-null prior
bma_lwage_red2 = bas.lm(lwage ~ ., data = wage_red,
                   prior = "ZS-null", 
                   modelprior = beta.binomial(1, 1))

bma_lwage_red2

summary(bma_lwage_red2)
```
...
# Obtain the coefficients from the model `bma_lwage_red`
coef_lwage_red <- coefficients(bma_lwage_red)

# `age` is the 8th variable in the data set
...
there is a 95% chance that median wages increases 3.65 to 6.95 units higher with one year more education.
...
variable.names(BPM_pred_lwage_red)

opt_bpm_red <- which.max(BPM_pred_lwage_red$fit)
...
ci_lwage_red[opt_bpm_red,]

exp(ci_lwage_red[opt_bpm_red,])
```

using the reduced model, a 95% prediction interval is 716-2965
