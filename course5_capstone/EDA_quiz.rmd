...
ames_train %>% 
  ...
  summarise(avg = mean(price, na.rm=TRUE), std = sd(price, na.rm=TRUE)) %>% 
  arrange(desc(std))

# boxplot to visualize price distribution per neighborhood
...
ggplot(data = ames_train, aes(x = Lot.Area, y = price/1000)) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(title="Prices by lot area", x="Lot area", y="Prices in K")

ggplot(data = ames_train, aes(x = Bedroom.AbvGr, y = price/1000)) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(title="Prices by bedroom above grade", x="Bedroom above grade", y="Prices in K")

ggplot(data = ames_train, aes(x = Overall.Qual, y = price/1000)) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(title="Prices by overall quality", x="Overall quality", y="Prices in K")

ggplot(data = ames_train, aes(x = Year.Built, y = price/1000)) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(title="Prices by year built", x="Year built", y="Prices in K")
```
...
ggplot(data = ames_train, aes(x = area, y = price/1000)) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(title="Prices by area_1", x="area", y="Prices in K")

ggplot(data = ames_train, aes(x = area, y = log(price))) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(title="Prices by area_2", x="area", y="log(Price)")

ggplot(data = ames_train, aes(x = log(area), y = price/1000)) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(title="Prices by area_3", x="log(area)", y="Prices in K")

ggplot(data = ames_train, aes(x = log(area), y = log(price))) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(title="Prices by area_4", x="log(area)", y="log(Price)")
```
...
# if the prior is $Beta(\alpha,\beta)$, then the posterior with $x$ successes in $n trials$ is $Beta(\alpha+x,\beta+n-x)$
```
...
ames_train <- ames_train %>% 
  mutate(has_garage = ifelse(is.na(Garage.Type), "no", "yes"))

inference(data=ames_train, y=area, x=has_garage, type="ht", alternative = "twosided",
          method="theoretical", null=0, statistic="mean")
```
...
# lambda = 3, std = 1
# k = (lambda/std)^2, theta = std^2/lambda
# k_star = k + sum(xi)
...
sum_x <- ames_train %>% 
  filter(area > 2000) %>%
  summarise(sum_x = sum(Bedroom.AbvGr))
...
c(sum_x, n, k_star, theta_star)

lambda_star <- k_star*theta_star
std_star = theta_star*sqrt(k_star)

c(lambda_star, std_star)
```
...
# outliers obtained from plot
lm_q10 = lm(log(price) ~ log(area), data = ames_train)
par(mfrow = c(2,2))
plot(lm_q10)

# find outliers by setting threshold of R_squared at the (n-3)th biggest R_squared
rsquared_q10 = resid(lm_q10)**2
threshold = sort(rsquared_q10, partial=(1000-3))[997]
which(rsquared_q10 > threshold)
...
ames_train %>% group_by(Neighborhood) %>% 
  summarise(mean_Bldg_Type = mean(Bldg.Type == "1Fam")) %>%
  filter(mean_Bldg_Type == 1)
