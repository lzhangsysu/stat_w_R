...
m_q1 <- lm(log(price) ~ log(area) + log(Lot.Area) + Bedroom.AbvGr + Overall.Qual + Land.Slope, data = df_1)

step_AIC <- stepAIC(m_q1, k = 2)

anova(step_AIC)

step_BIC <- stepAIC(m_q1, k = log(nrow(na.omit(df_1))))
...
m_q2_1 <- lm(log(price) ~ Bedroom.AbvGr, data = df_1)

m_q2_2 <- lm(log(price) ~ Bedroom.AbvGr + log(area), data = df_1)
...
m_q3 <- lm(log(price) ~ log(area), data = df_1)

pred_q3 = predict(m_q3, newdata = ames_train)

ames_train$residual = resid(m_q3)

ames_train %>%
  group_by(Neighborhood) %>%
  summarise(avg_res = mean(residual)) %>%
...
ames_train %>%
  group_by(Neighborhood) %>%
  summarise(mean_r_sq = mean(residual^2)) %>%
  arrange(desc(mean_r_sq))
```
...
# We can use the information that a house does not have a basement or garage to create a separate category for `NA` values.  
# This will allow us to avoid discarding observations and use the model to predict prices for out-of-sample houses that lack garages or basements.
```
...
m_q7 <- lm(log(price) ~ Overall.Cond + Overall.Qual, data = ames_train)

pred_q7 = predict(m_q7, newdata = ames_train)

ames_train$pred_q7 = exp(pred_q7)

ames_train %>%
  group_by(MS.SubClass) %>%
  summarise(med_pred_q7 = median(pred_q7)) %>%
  arrange(desc(med_pred_q7))
```
...
m_q9 <- lm(log(price) ~ Bedroom.AbvGr, ames_train)
...
m_q10 <- lm(log(price) ~ Sale.Condition, data = ames_train)
summary(m_q10)
```
...
df_4 <- subset(ames_train, Sale.Condition == 'Normal')

nrow(df_4)/nrow(ames_train)
```
...
m_q12 <- lm(log(price) ~ log(area), data = df_4)

summary(m_q3)$adj.r.squared

summary(m_q12)$adj.r.squared
```
