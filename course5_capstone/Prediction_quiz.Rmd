...
pred_q1 = exp(predict(model.AIC, newdata = ames_test))
...
pred_full_train <- exp(predict(model.full, ames_train))
resid_full_train <- ames_train$price - pred_full_train
rmse_full_train <- sqrt(mean(resid_full_train^2))
rmse_full_train

# RMSE for ames_train, model.AIC
pred_AIC_train <- exp(predict(model.AIC, ames_train))
resid_AIC_train <- ames_train$price - pred_AIC_train
rmse_AIC_train <- sqrt(mean(resid_AIC_train^2))
rmse_AIC_train

# RMSE for ames_test, model.full
pred_full_test <- exp(predict(model.full, ames_test))
resid_full_test <- ames_test$price - pred_full_test
rmse_full_test <- sqrt(mean(resid_full_test^2))
rmse_full_test

# RMSE for ames_test, model.AIC
pred_AIC_test <- exp(predict(model.AIC, ames_test))
resid_AIC_test <- ames_test$price - pred_AIC_test
rmse_AIC_test <- sqrt(mean(resid_AIC_test^2))
rmse_AIC_test
```
...
pred.test.MPM <- predict(model.bas, newdata = ames_test, estimator="MPM")
pred.MPM.rmse <- sqrt(mean((exp(pred.test.MPM$fit) - ames_test$price)^2))
pred.MPM.rmse
...
out_mpm = as.data.frame(cbind(exp(confint(pred.test.MPM)),
                          price = ames_test$price))

# Fix names in dataset
colnames(out_mpm)[1:2] <- c("lwr", "upr")  #fix names

# Get Coverage
pred.test.MPM.coverage <- out_mpm %>% summarize(cover = sum(price >= lwr & price <= upr)/n())
